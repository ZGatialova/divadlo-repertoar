...
$stmt = $pdo->query("SELECT id_hry, nazov FROM hry ORDER BY nazov");
$hry = $stmt->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "INSERT INTO predstavenia (id_hry, datum, cas, miesto) VALUES (?, ?, ?, ?)"
        );
        $stmt->execute([$id_hry, $datum, $cas, $miesto !== '' ? $miesto : null]);
...
<head>
    <meta charset="UTF-8">
    <title>Pridať predstavenie</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">

<div class="container py-5" style="max-width: 700px;">
    <h1 class="h3 mb-4">Pridať predstavenie</h1>
